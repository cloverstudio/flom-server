...
async function getCustomerActivationData({ dateLimit, noSpending = false }) {
  const { properties: customerActivationData = {} } =
    (await Configuration.findOne({ type: "general", name: "customer-activation" }).lean()) || {};

  if (noSpending) return customerActivationData;

  dateLimit = dateLimit === undefined ? customerActivationData.lastResetDate : dateLimit;
