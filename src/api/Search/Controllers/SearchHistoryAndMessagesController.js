...
const { searchHistory, searchMessage } = require("#logics");

/**
      * @api {get} /api/v2/search/historyAndMessages?keyword=keyword Search history and messages
      * @apiName SearchAll
      * @apiGroup WebAPI
...
router.get("/", auth({ allowUser: true }), async function (request, response) {
  try {
    ...
    const histories = await searchHistory(0, 0, keyword, user, Const.pagingRowsHistoryAndMessages);
    result.histories = histories;

    const messages = await searchMessage(user, keyword, 0, Const.pagingRowsHistoryAndMessages);
