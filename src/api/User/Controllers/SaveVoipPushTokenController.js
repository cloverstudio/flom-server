...
router.post("", auth({ allowUser: true }), async function (request, response) {
  try {
    ...
    await updateUsersPushToken(request.body.pushToken, user._id.toString(), true);

    if (!user.voipPushToken.includes(request.body.pushToken)) {
      user.voipPushToken.push(request.body.pushToken);
    }
    ...
    if (UUID && newPushToken && savedUUIDs) {
      savedUUIDs.forEach((o) => {
        if (o.UUID == UUID) {
          ...
          if (!o.voipPushTokens.includes(newPushToken)) {
            o.voipPushTokens.push(newPushToken);
            changed = true;
          }
        }
      });
