...
const { Tribe, User } = require("#models");
const { formatUserDetailsResponse } = require("#logics");

/**
 * @api {get} /api/v2/user/tribe-members Get all members of users tribes flom_v1
 * @apiVersion 2.0.22
 * @apiName  Get all members of users tribes flom_v1
 * @apiGroup WebAPI User
 * @apiDescription  Get all members of users tribes.
 *
...
router.get("/", auth({ allowUser: true }), async function (request, response) {
  try {
    ...
    const tribes = await Tribe.find({ ownerId: userId }).lean();
    let memberIds = [];
    ...
    for (let i = 0; i < members.length; i++) {
      await formatUserDetailsResponse({ user: members[i], requestAccessToken: token });
    }
    ...
    const paginatedMembers =
      total === 0 ? [] : members.slice((page - 1) * size, (page - 1) * size + size);
    ...
    Base.newErrorResponse({
      response,
      message: "GetAllMembersOfUsersTribesController",
      error,
    });
  }
});
