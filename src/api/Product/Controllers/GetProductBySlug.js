...
/**
 * @api {get} /api/v2/product/getbyslug Get Product By Slug
 * @apiVersion 2.0.34
 * @apiName Get Product By Slug
 * @apiGroup WebAPI Products
 * @apiDescription Get Product By Slug. If product is from Recombee recommendation send the recommId as well.
 *
 * @apiHeader {String} access-token Users unique access-token.
 *
 * @apiParam (Query Parameter) {String} slug          product slug
 * @apiParam (Query Parameter) {String} [recommId]    recommId
 *
...
router.get("/", async function (request, response) {
  try {
    const { slug, recommId = null } = request.query;
