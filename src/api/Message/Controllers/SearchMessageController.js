...
const { searchMessage } = require("#logics");
...
router.get("/:keyword/:page", auth({ allowUser: true }), async function (request, response) {
  try {
    const keyword = decodeURIComponent(request.params.keyword);
    const page = +request.params.page - 1;

    const { messages = [], count = 0 } = await searchMessage(
      request.user,
      keyword,
      page,
      Const.pagingRows,
